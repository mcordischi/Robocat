    ...
    public void run() {
        ...
        while (mDoProcess && mCamera != null) {
            ...
            if (grabbed) {
                ...
                Imgproc.cvtColor(mCurrentFrame, mCurrentFrameGray, Imgproc.COLOR_RGB2HSV_FULL);


                //Red is in the Hue=0, we must move the limit
                int rotation = 128 - 255; // 255= red
                Core.add(mCurrentFrameGray,new Scalar(rotation,0,0),mCurrentFrameGray);

                //InRange ThresHolding
                Core.inRange(mCurrentFrameGray,LOWER_IN_RANGE,UPPER_IN_RANGE,mThreshFrameResult);
